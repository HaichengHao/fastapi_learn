...
#tips:定义一个函数检查token
def  check_user_permission(token:Annotated[str,Header(...)]): #tips:回顾一下，点点点是未写参数但是必填的意思
...
@app.get('/items/')
async def  get_items(pagination:Annotated[int,Depends(get_pagination_params)]):
    return {
