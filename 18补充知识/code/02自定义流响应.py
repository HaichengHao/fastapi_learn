...
"""
'''
在上面的示例中，字节数据被流式传输了，但响应没有 Content-Type 标头，因此客户端不知道它接收的是什么类型的数据。

你可以创建一个 StreamingResponse 的自定义子类，将 Content-Type 标头设置为你正在流式传输的数据类型。

例如，你可以创建一个 PNGStreamingResponse，通过 media_type 属性将 Content-Type 标头设置为 image/png。
'''
import uvicorn
import base64
...
binary_image = base64.b64decode(image_base64) #tips:base64.b64decode返回的是一个byte对象
