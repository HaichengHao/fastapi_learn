...
async def create_emp(employee: Empschema, session: Annotated[Session, Depends(get_session)]):
    ...
    # important:这里和tortoiseorm差不多,employee是pydantic类型,用.model_dump是转换成字典
    #  然后**employee.model_dump()就是解包赋值
...
## 修改操作  
一定要注意传入的`exclude_unset`,它可以只将我们需要更新的属性进行更新,别的不动 ⚠️⚠️
```python   
# tips:修改员工信息,通过id进行修改

@emp_apirt.put('/emp/{id}', response_model=Empschema)
async def mofy_emp(id:Annotated[int,Path(description='员工id')],session:Annotated[Session,Depends(get_session)],employee:Empschema):
    employee.id=id  #tips:因为不对id进行修改,所以这里直接先给其赋值
    target_emp = session.execute(Select(Employee.name).where(Employee.id  == id)).first()
    if target_emp==None:
        raise HTTPException(status_code=405,detail='未查讯到员工信息,请检查id')
    session.execute(update(Employee),[employee.model_dump(exclude_unset=True)]) 
